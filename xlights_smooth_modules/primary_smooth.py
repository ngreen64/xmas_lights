...
def run_once_at_start():
   ...
   this_falloff = random.uniform(float(default_light_falloff.split("-")[0]),float(default_light_falloff.split("-")[1]))
   print(str(this_falloff) + " <- this falloff")
   y = random.randint(1,3)
   if y == 1:
      if isinstance(default_light_falloff, float):
         colours_in_play[com_var.timenow] = (255,0,0,direction,default_light_falloff,time_for_run)
      else:
         colours_in_play[com_var.timenow] = (255,0,0,direction,random.uniform(float(default_light_falloff.split("-")[0]),float(default_light_falloff.split("-")[1])),time_for_run)
   elif y == 2:
      if isinstance(default_light_falloff, float):
         colours_in_play[com_var.timenow] = (0,255,0,direction,default_light_falloff,time_for_run)
      else:
         colours_in_play[com_var.timenow] = (0,255,0,direction,random.uniform(float(default_light_falloff.split("-")[0]),float(default_light_falloff.split("-")[1])),time_for_run)
   else:
      if isinstance(default_light_falloff, float):
         colours_in_play[com_var.timenow] = (0,0,255,direction,default_light_falloff,time_for_run)
      else:
         colours_in_play[com_var.timenow] = (0,0,255,direction,random.uniform(float(default_light_falloff.split("-")[0]),float(default_light_falloff.split("-")[1])),time_for_run)
   ...
   for colour_time in list(colours_in_play):
     ...
     if colours_in_play[colour_time][3] == "positive":
        try: 
           ...
           print("error 1")
           print(type(com_var.timenow))
           print(type(colour_time))
           print(type(this_light_time_for_run))
           print(type(com_var.no_of_lights))
           print(str(type(light_span)) + " : " + light_span)
           print(colours_in_play)
     ...
     if colours_in_play[colour_time][3] == "negative":
        try: 
           position = com_var.no_of_lights - ((com_var.timenow - colour_time)/1000)/this_light_time_for_run*(com_var.no_of_lights+light_span*2) + light_span
        except:
           print("error 2")
           print(type(com_var.timenow))
           print(type(colour_time))
